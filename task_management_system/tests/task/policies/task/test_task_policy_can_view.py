...
class TestCanView:
    ...
    def test_permission_granted_member(self, actor_fixture, request, task, caplog_loguru):
        actor, actor_membership = request.getfixturevalue(actor_fixture)
        result = TaskPolicy.can_view(actor, task)
        ...
        assert_log(
            log_record,
            "Permission granted to view task: True",
            "DEBUG",
            task=task.id,
    ...
    def test_permission_granted_superuser(self, superuser, task, caplog_loguru):
        result = TaskPolicy.can_view(superuser, task)
        ...
        assert_log(
            log_record,
            "Permission granted to view task: Actor is superuser.",
            "INFO",
            task=task.id,
        )

    def test_permission_denied(self, user_is_not_member, task, caplog_loguru):
        ...
        assert_log(
            log_record,
            "Permission denied to view task: User has no membership for task.",
            "WARNING",
            task=task.id,
        )
    ...
    def test_denied_unsupported_future_role(self, member_is_owner, task, caplog_loguru):
        ...
        with pytest.raises(RuntimeError):
            TaskPolicy.can_view(actor, task)
