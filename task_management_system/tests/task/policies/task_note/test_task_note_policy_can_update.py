...
class TestCanUpdate:
    ...
    def test_permission_granted_by_role(self, actor_fixture, request, task_note, caplog_loguru):
        actor, actor_membership = request.getfixturevalue(actor_fixture)
        task_note.created_at = timezone.now() - timedelta(hours=5)
        task_note.save()

        result = TaskNotePolicy.can_update(actor, task_note, {"note"})
    ...
    def test_permission_granted_actor_is_author(self, task_note, caplog_loguru):
        ...
        with freeze_time(timezone.now()):
            result = TaskNotePolicy.can_update(task_note.author, task_note, {"note"})
        assert result is True
    ...
    def test_permission_granted_superuser(self, superuser, task_note, caplog_loguru):
        result = TaskNotePolicy.can_update(superuser, task_note, {"note"})
    ...
    def test_permission_denied_by_role(self, actor_fixture, request, task_note, caplog_loguru):
        actor, actor_membership = request.getfixturevalue(actor_fixture)
        task_note.created_at = timezone.now() - timedelta(hours=5)
        task_note.save()
    ...
    def test_permission_denied_restricted_fields(self, member_is_owner, task_note, caplog_loguru):
        actor, actor_membership = member_is_owner
        requested_fields = {"note", "author_id"}

        result = TaskNotePolicy.can_update(actor, task_note, requested_fields)
    ...
    def test_denied_unsupported_future_role(self, member_is_owner, task_note, caplog_loguru):
        ...
        with pytest.raises(RuntimeError):
            TaskNotePolicy.can_update(actor, task_note, {"note"})
