...
class TestPagination:
    def test_first_page(self, superuser, tasks, api_client, monkeypatch):
    ...
    def test_n_page(self, superuser, tasks, api_client, monkeypatch):
        url = parse_url(BASENAME, "list")
    ...
    def test_last_page(self, superuser, tasks, api_client, monkeypatch):
        url = parse_url(BASENAME, "list")
    ...
    def test_memberships_first_page(
        self, superuser, task_memberships, task, api_client, monkeypatch
    ):
        ...
        assert response.status_code == 200
        assert_paginator(response.data)
        assert response.data["count"] == TaskMembership.objects.filter(task=task.id).count()
        assert len(response.data["results"]) == 1
        assert response.data["previous"] is None
        assert response.data["next"].endswith("/?page=2")

    def test_memberships_n_page(self, superuser, task_memberships, task, api_client, monkeypatch):
        url = reverse(f"{BASENAME}-memberships", args=[task.id])
        monkeypatch.setattr("task.api.v1.paginator.TaskMembershipPagination.page_size", 1)
    ...
    def test_memberships_last_page(
        ...
        monkeypatch.setattr("task.api.v1.paginator.TaskMembershipPagination.page_size", 1)
        cnt = TaskMembership.objects.filter(task=task.id).count()
    ...
    def test_notes_first_page(self, superuser, task_notes, task, api_client, monkeypatch):
        ...
        assert response.status_code == 200
        assert_paginator(response.data)
        assert response.data["count"] == TaskNote.objects.filter(task=task.id).count()
        assert len(response.data["results"]) == 1
        assert response.data["previous"] is None
        assert response.data["next"].endswith("/?page=2")

    def test_notes_n_page(self, superuser, task_notes, task, api_client, monkeypatch):
        url = reverse(f"{BASENAME}-notes", args=[task.id])
    ...
    def test_notes_last_page(self, superuser, task_notes, task, api_client, monkeypatch):
        url = reverse(f"{BASENAME}-notes", args=[task.id])

        monkeypatch.setattr("task.api.v1.paginator.TaskNotePagination.page_size", 1)
        cnt = TaskNote.objects.filter(task=task.id).count()
