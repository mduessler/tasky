...
class TestCanViewTasksOfUser:
    ...
    def test_user_can_view_itself(self, actor_fixture, request, caplog_loguru):
        ...
        result = TmsUserPolicy.can_view_tasks_of_user(actor, actor)
        ...
        assert_log(
            log_record,
            "Permission granted: User can view it's own tasks.",
            "DEBUG",
            user=actor.id,
        )
    ...
    def test_superuser_can_view_users(
        ...
        assert_log(
            log_record,
            "Permission granted: Actor is superuser and can view user tasks.",
            "INFO",
            user=user.id,
        )
    ...
    def test_user_can_not_view_users(self, actor_fixture, user_fixture, request, caplog_loguru):
        ...
        actor, _ = request.getfixturevalue(actor_fixture)
        user, _ = request.getfixturevalue(user_fixture)
        result = TmsUserPolicy.can_view_tasks_of_user(actor, user)
        ...
        assert_log(
            log_record,
            "Permission denied: Actor is not allowed to view user tasks.",
            "DEBUG",
            user=user.id,
        )
